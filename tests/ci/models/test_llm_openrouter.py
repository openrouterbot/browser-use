...
async def test_extra_body_is_forwarded_via_the_sdk_extra_body_param():
	extra_body = {
	...
	# Captured before patching: create() is keyword-only with no **kwargs, so OpenRouter-only
	# keys are deliverable only through the SDK's own extra_body parameter.
...
async def test_unset_extra_body_is_not_sent():
	llm = ChatOpenRouter(model='openai/gpt-4o', api_key='test-key')
	client = llm.get_client()
	...
	assert create.await_args_list[0].kwargs['extra_body'] is None
